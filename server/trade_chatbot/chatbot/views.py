...
def chat(request):
    try:
        ...
        # Create job — worker service will pick it up
        return _handle_trade_request(data_type, country_keyword, user_year)
...
def _handle_trade_request(data_type, country, user_year):
    ...
    # Create job record — the worker process picks this up automatically
    ...
    return Response({
        "message": f"🔍 Fetching {data_type} data for '{country}' ({user_year})... This may take a few minutes.",
        "source": "processing",
    })
